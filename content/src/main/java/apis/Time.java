...
public class Time {
  ...
  public static void main(String[] args) {
    ...
    */
    ...
    println("Nanoseconds between the two instants: " + now.until(then, ChronoUnit.NANOS));
    // Or
    var duration = Duration.between(now, then);
    println("Nanoseconds between the two instants: " + duration.toNanos());
    ...
    println("Days between the two dates: " + period.getDays()); // ERRRRR, it's 1 !?

    // To get amount of timeunits better use
    var daysBetween = ChronoUnit.DAYS.between(today, tomorrowNextYear);
    println("Days between the two dates: " + daysBetween); // Ahhhhh

    // And to access it's properties
    println(zonedDateTime.getDayOfMonth());
